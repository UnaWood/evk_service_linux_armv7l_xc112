 ...
 */
static void acc_driver_os_localtime(struct tm *time_tm, uint32_t *time_usec)
{
 ...
 */
static acc_os_thread_handle_t acc_driver_os_thread_create(void (*func)(void *param), void *param)
{
	int	ret;
 ...
 */
static void acc_driver_os_thread_delete(void)
{
	pthread_exit(NULL);
}
 ...
 */
static bool acc_driver_os_thread_cleanup(acc_os_thread_handle_t thread)
{
 ...
 */
static void *acc_driver_os_dynamic_open(char *filename)
{
 ...
 *
 * @param  handle The handle returned from acc_driver_os_dynamic_open()
 */
static void acc_driver_os_dynamic_close(void *handle)
{
	dlclose(handle);
}
 ...
 */
static void *acc_driver_os_dynamic_symbol(void *handle, char *name)
{
	void	*addr;
 ...
 *
 * @param  handle The handle returned by acc_driver_os_dynamic_open()
 * @return A pointer to the error string
 */
static char *acc_driver_os_dynamic_error(void *handle __attribute__((unused)))
{
	return dlerror();
}
 ...
 */
static int acc_driver_os_net_hostname_to_ip(const char *hostname , char* buf, size_t buflen)
{
	struct hostent *he;
	int res = -1;
...
static acc_os_net_address_t acc_driver_os_net_string_to_address(const char *str)
{
	struct in_addr	addr;
	...
	if (!acc_driver_os_net_hostname_to_ip(str, ip, sizeof(ip))) {
...
static void acc_driver_os_net_address_to_string(acc_os_net_address_t address, char *buffer, size_t max_size)
{
	struct in_addr	addr;
...
static void acc_driver_os_net_disconnect(acc_os_socket_t sock)
{
	if (sock != NULL) {
		close(sock->handle);
	}
...
static acc_os_socket_t acc_driver_os_net_connect(acc_os_net_address_t address, acc_os_net_port_t port)
{
	...
	if ((connect(sock->handle, (struct sockaddr*)&addr, sizeof(addr)) < 0) && (errno != EINPROGRESS)) {
		char name[INET_ADDRSTRLEN];
		acc_driver_os_net_address_to_string(address, name, sizeof(name));
		ACC_LOG_WARNING("%s: connect(%s:%u): (%u) %s", __func__, name, port, errno, strerror(errno));
		acc_driver_os_net_disconnect(sock);
		return NULL;
	}
	...
	if (setsockopt(sock->handle, IPPROTO_TCP, TCP_NODELAY, (void*)&value, sizeof(value)) < 0) {
		ACC_LOG_WARNING("%s: setsockopt(TCP_NODELAY): (%u) %s", __func__, errno, strerror(errno));
		acc_driver_os_net_disconnect(sock);
		return NULL;
	}
...
static void acc_driver_os_set_socket_invalid(acc_os_socket_t sock)
{
	if (sock != NULL) {
...
static bool acc_driver_os_is_socket_valid(acc_os_socket_t sock)
{
	bool valid = false;
...
static int acc_driver_os_net_send(acc_os_socket_t sock, void *buffer, size_t size)
{
	ssize_t	result;
	ssize_t	remain = size;
...
static int acc_driver_os_net_receive(acc_os_socket_t sock, void *buffer, size_t max_size, uint_fast32_t timeout_us)
{
	ssize_t	result;
...
static int_fast8_t acc_driver_os_semaphore_wait(acc_os_semaphore_t sem, uint_fast16_t timeout_ms)
{
	struct timespec ts;
	struct timeval tv;
...
static uint16_t acc_driver_os_ntohs(uint16_t value)
{
	return ntohs(value);
}


static uint16_t acc_driver_os_htons(uint16_t value)
{
	return htons(value);
}


static uint32_t acc_driver_os_ntohl(uint32_t value)
{
	return ntohl(value);
}


static uint32_t acc_driver_os_htonl(uint32_t value)
{
	return htonl(value);
}
...
void acc_driver_os_linux_register(void)
{
	...
	acc_device_os_get_mem_alloc_func			= malloc;
	acc_device_os_get_mem_free_func				= free;
	acc_device_os_get_thread_id_func			= acc_driver_os_get_thread_id;
	acc_device_os_localtime_func				= acc_driver_os_localtime;
	...
	acc_device_os_thread_delete_func			= acc_driver_os_thread_delete;
	acc_device_os_thread_cleanup_func			= acc_driver_os_thread_cleanup;
	acc_device_os_dynamic_open_func				= acc_driver_os_dynamic_open;
	acc_device_os_dynamic_close_func			= acc_driver_os_dynamic_close;
	acc_device_os_dynamic_symbol_func			= acc_driver_os_dynamic_symbol;
	acc_device_os_dynamic_error_func			= acc_driver_os_dynamic_error;
	acc_device_os_net_string_to_address_func		= acc_driver_os_net_string_to_address;
	acc_device_os_net_address_to_string_func		= acc_driver_os_net_address_to_string;
	acc_device_os_net_connect_func				= acc_driver_os_net_connect;
	acc_device_os_net_disconnect_func			= acc_driver_os_net_disconnect;
	acc_device_os_net_send_func				= acc_driver_os_net_send;
	acc_device_os_net_receive_func				= acc_driver_os_net_receive;
	acc_device_os_set_socket_invalid_func			= acc_driver_os_set_socket_invalid;
	acc_device_os_is_socket_valid_func			= acc_driver_os_is_socket_valid;
	...
	acc_device_os_ntohs_func				= acc_driver_os_ntohs;
	acc_device_os_htons_func				= acc_driver_os_htons;
	acc_device_os_ntohl_func				= acc_driver_os_ntohl;
	acc_device_os_htonl_func				= acc_driver_os_htonl;
}
